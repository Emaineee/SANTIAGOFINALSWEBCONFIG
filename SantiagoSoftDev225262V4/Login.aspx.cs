    ...
    public partial class Login : System.Web.UI.Page
    {
        ...
        protected void btnLogIn_Click(object sender, EventArgs e)
        {
            ...
            using (OleDbConnection conn = new OleDbConnection(connStr))
            {
                ...
                OleDbCommand cmd = new OleDbCommand("SELECT studIndex, UN, PW FROM tblUNPW WHERE UN = ?", conn);
                ...
                if (validLogin)
                {
                    ...
                    {
                        dr.Close();
                        OleDbCommand upd = new OleDbCommand("UPDATE tblUNPW SET PW = ? WHERE studIndex = ?", conn);
                        upd.Parameters.Add("?", OleDbType.VarChar).Value = hashedPW;
                        upd.Parameters.Add("?", OleDbType.VarChar).Value = dbStudIndex;
                        upd.ExecuteNonQuery();
                    }
        ...
        protected void btnSignUp_Click(object sender, EventArgs e)
        {
            ...
            {
                conn.Open();
                OleDbCommand checkCmd1 = new OleDbCommand("SELECT COUNT(*) FROM tblUNPW WHERE UN = ?", conn);
                checkCmd1.Parameters.Add("?", OleDbType.VarChar).Value = username;
                if (Convert.ToInt32(checkCmd1.ExecuteScalar()) > 0)
                { lblSignUpMessage.Text = "Username already exists."; return; }

                try
                {
                    OleDbCommand checkCmd2 = new OleDbCommand("SELECT COUNT(*) FROM tblUNPW WHERE Email = ?", conn);
                    checkCmd2.Parameters.Add("?", OleDbType.VarChar).Value = email;
                    if (Convert.ToInt32(checkCmd2.ExecuteScalar()) > 0)
                    { lblSignUpMessage.Text = "Email already registered."; return; }
                }
                catch { /* Email column might not exist */ }
            }
        ...
        protected void btnRegisterFinal_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                int yearLevel = 1;
                int.TryParse(data["yearLevel"]?.ToString(), out yearLevel);
                if (yearLevel < 1 || yearLevel > 4) yearLevel = 1;

                int progId = 0;
                int.TryParse(data["programId"]?.ToString(), out progId);
                if (progId <= 0) throw new Exception("Please select a valid program.");
                ...
                if (string.IsNullOrWhiteSpace(sex)) throw new Exception("Sex is required.");

                using (OleDbConnection conn = new OleDbConnection(connStr))
                {
                    conn.Open();

                    // 1. Insert into tblUNPW (UN and PW only first)
                    using (OleDbCommand cmd1 = new OleDbCommand(
                        "INSERT INTO tblUNPW (UN, PW) VALUES (?, ?)", conn))
                    {
                        cmd1.Parameters.Add("?", OleDbType.VarChar).Value = username;
                        cmd1.Parameters.Add("?", OleDbType.VarChar).Value = HashPassword(password);
                    ...
                    try
                    {
                        using (OleDbCommand cmdUpd = new OleDbCommand(
                            "UPDATE tblUNPW SET Email = ? WHERE UN = ?", conn))
                        {
                            cmdUpd.Parameters.Add("?", OleDbType.VarChar).Value = email;
                            cmdUpd.Parameters.Add("?", OleDbType.VarChar).Value = username;
                            cmdUpd.ExecuteNonQuery();
                        }
                    }
                    catch { /* Email column might not exist */ }

                    // Update studIndex if column exists
                    try
                    {
                        using (OleDbCommand cmdUpd = new OleDbCommand(
                            "UPDATE tblUNPW SET studIndex = ? WHERE UN = ?", conn))
                        {
                            cmdUpd.Parameters.Add("?", OleDbType.VarChar).Value = studIndexInput;
                            cmdUpd.Parameters.Add("?", OleDbType.VarChar).Value = username;
                            cmdUpd.ExecuteNonQuery();
                        }
                    }
                    catch { /* studIndex column might not exist */ }

                    // 2. Insert into tbl_StudentInfo - truncate fields to fit Access limits
                    try
                    {
                        string sID = studIndexInput.Length > 20 ? studIndexInput.Substring(0, 20) : studIndexInput;
                        string fName = firstName.Length > 20 ? firstName.Substring(0, 20) : firstName;
                        string lName = lastName.Length > 20 ? lastName.Substring(0, 20) : lastName;
                        string sSex = sex.Length > 6 ? sex.Substring(0, 6) : sex;
                        ...
                        {
                            cmd2.Parameters.Add("?", OleDbType.VarChar).Value = sID;
                            cmd2.Parameters.Add("?", OleDbType.VarChar).Value = fName;
                            cmd2.Parameters.Add("?", OleDbType.VarChar).Value = DBNull.Value;
                            cmd2.Parameters.Add("?", OleDbType.VarChar).Value = lName;
                            cmd2.Parameters.Add("?", OleDbType.Date).Value = birthDate;
                            cmd2.Parameters.Add("?", OleDbType.VarChar).Value = sSex;
                    ...
                    catch (Exception ex2)
                    {
                        ...
                        new OleDbCommand("DELETE FROM tblUNPW WHERE UN = '" + username.Replace("'", "''") + "'", conn).ExecuteNonQuery();
                        throw new Exception("Error inserting to tbl_StudentInfo: " + ex2.Message);
                    }
                }

                Session["User"] = username;
                Session["StudIndex"] = studIndexInput;
                Session["LoggedIn"] = true;
                Session["OTPVerified"] = false;
                Session["OTP"] = null;
        ...
        protected void btnSendResetOTP_Click(object sender, EventArgs e)
        {
            ...
            {
                conn.Open();
                OleDbCommand cmd = new OleDbCommand("SELECT studIndex, UN, Email FROM tblUNPW WHERE UN = ?", conn);
                cmd.Parameters.Add("?", OleDbType.VarChar).Value = input;
        ...
        protected void btnResetPassword_Click(object sender, EventArgs e)
        {
            ...
            using (OleDbConnection conn = new OleDbConnection(connStr))
            {
                ...
                cmd.Parameters.Add("?", OleDbType.VarChar).Value = hashedPW;
                cmd.Parameters.Add("?", OleDbType.VarChar).Value = Session["ResetUser"].ToString();
...
<body>
  <div class='container'>
    ...
    <div class='body'>
      ...
      <br>
      <span class='badge'>Expires in {2} minutes</span>
...
</html>", username, otp, OTP_EXPIRY_MINUTES);
